...
int main(void){
    ...
    cJSON* adding_obj = cJSON_CreateObject();
    cJSON* adding = cJSON_CreateString("Gholi!");
    cJSON_AddItemToObject(adding_obj,"content",adding);
    cJSON* str = cJSON_CreateArray();
    cJSON_AddItemToObject(obj,"message",str);
    cJSON_AddItemToArray(str, adding_obj);
    printf("%s", obj->valuestring);
    cJSON* jsonptr = cJSON_GetObjectItem(obj,"message");
    char *ptr = cJSON_Print(obj);
    printf("%s", jsonptr->valuestring);
    cJSON* jsonptr2 = cJSON_GetArrayItem(str,0);
    printf("%s", jsonptr2->valuestring);
